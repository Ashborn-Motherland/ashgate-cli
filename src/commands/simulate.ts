...
import { projectsApi, billingPlansApi } from '../api/client';
...
export function registerSimulateCommands(program: Command) {
    ...
    simulateCmd
        ...
        .action(async (options) => {
            console.log(chalk.blue('Simulateur de Webhook FedaPay 🚀'));
            ...
            try {
                ...
                console.error(chalk.yellow('⚠ Impossible de récupérer les projets depuis le serveur. Êtes-vous connecté ?'));
            ...
            if (projects.length > 0) {
                ...
                if (projectSelection.projectId === 'manual') {
                    const manualProject = await inquirer.prompt([
                        {
                            type: 'input',
                            name: 'projectId',
                ...
                const manualProject = await inquirer.prompt([
                    {
                        ...
                        validate: (input: string) => input.trim().length > 0 || 'Veuillez entrer un ID de projet',
                    }
                ]);
                projectId = manualProject.projectId;
            }
            ...
            if (projectId) {
                try {
                    plans = await billingPlansApi.list(projectId);
                } catch (e) {
            ...
            if (plans.length > 0) {
                const planSelection = await inquirer.prompt([
                    {
                        type: 'list',
                        name: 'planId',
                ...
                const manualPlan = await inquirer.prompt([
                    {
                        type: 'input',
                        name: 'planId',
            ...
            const remainingAnswers = await inquirer.prompt([
                ...
                },
                {
                    ...
                    validate: (input: string) => !isNaN(parseInt(input)) || 'Veuillez entrer un nombre valide'
                },
                {
                    type: 'input',
                    name: 'txId',
            ...
            if (answers.planId)         customMetadata.ash_plan_id = answers.planId;
            if (answers.subscriptionId) customMetadata.ash_subscription_id = answers.subscriptionId;
            // If plan ID is provided, tag this as a subscription payment
            if (answers.planId)         customMetadata.ash_subscription_type = 'saas';
            ...
            if (options.secret) {
                ...
                const hmac = crypto.createHmac('sha256', options.secret).update(`${timestamp}.${payloadString}`).digest('hex');
                signature = `t=${timestamp},v1=${hmac}`;
            }

            console.log(chalk.dim(`\nEnvoi de l'événement vers ${options.url}...`));
            if (answers.planId) {
                console.log(chalk.dim(`  → Projet  : ${answers.projectId}`));
                console.log(chalk.dim(`  → Plan    : ${answers.planId}`));
            }

            try {
                const headers: Record<string, string> = { 'Content-Type': 'application/json' };
                if (signature) headers['x-fedapay-signature'] = signature;

                const response = await axios.post(options.url, payloadString, { headers });
                console.log(chalk.green(`\n✓ Webhook envoyé avec succès ! Status: ${response.status}`));
            } catch (error: any) {
                console.error(chalk.red(`✗ Échec de l'envoi du webhook vers ${options.url}`));
                if (error.response) {
