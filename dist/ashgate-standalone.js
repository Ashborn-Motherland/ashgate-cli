  ...
  "node_modules/.pnpm/dotenv@17.3.1/node_modules/dotenv/lib/main.js"(exports2, module2) {
    ...
    function configDotenv(options) {
      ...
      for (const path5 of optionPaths) {
        try {
          const parsed = DotenvModule.parse(fs8.readFileSync(path5, { encoding }));
    ...
    function patch(fs8) {
      ...
      function patchLchmod(fs9) {
        ...
        fs9.lchmodSync = function(path4, mode) {
          var fd = fs9.openSync(path4, constants.O_WRONLY | constants.O_SYMLINK, mode);
          var threw = true;
          var ret;
          try {
      ...
      function patchLutimes(fs9) {
        if (constants.hasOwnProperty("O_SYMLINK") && fs9.futimes) {
          fs9.lutimes = function(path4, at, mt, cb) {
            fs9.open(path4, constants.O_SYMLINK, function(er, fd) {
              if (er) {
                if (cb) cb(er);
                return;
              }
          ...
          fs9.lutimesSync = function(path4, at, mt) {
            var fd = fs9.openSync(path4, constants.O_SYMLINK);
            var ret;
            var threw = true;
            try {
...
var require_make_dir = __commonJS({
  "node_modules/.pnpm/make-dir@3.1.0/node_modules/make-dir/index.js"(exports2, module2) {
    ...
    var makeDir = async (input2, options) => {
      checkPath(input2);
      options = processOptions(options);
      const mkdir = promisify6(options.fs.mkdir);
      const stat = promisify6(options.fs.stat);
      if (useNativeRecursiveOption && options.fs.mkdir === fs8.mkdir) {
        const pth = path4.resolve(input2);
        await mkdir(pth, {
          mode: options.mode,
          recursive: true
        });
      ...
      return make(path4.resolve(input2));
    };
    module2.exports = makeDir;
    module2.exports.sync = (input2, options) => {
      checkPath(input2);
      options = processOptions(options);
      if (useNativeRecursiveOption && options.fs.mkdirSync === fs8.mkdirSync) {
        const pth = path4.resolve(input2);
        fs8.mkdirSync(pth, {
          mode: options.mode,
          recursive: true
...
// node_modules/.pnpm/xdg-basedir@4.0.0/node_modules/xdg-basedir/index.js
var require_xdg_basedir = __commonJS({
  "node_modules/.pnpm/xdg-basedir@4.0.0/node_modules/xdg-basedir/index.js"(exports2) {
    "use strict";
    var os3 = require("os");
    var path4 = require("path");
    var homeDirectory = os3.homedir();
    var { env: env2 } = process;
    exports2.data = env2.XDG_DATA_HOME || (homeDirectory ? path4.join(homeDirectory, ".local", "share") : void 0);
    exports2.config = env2.XDG_CONFIG_HOME || (homeDirectory ? path4.join(homeDirectory, ".config") : void 0);
    exports2.cache = env2.XDG_CACHE_HOME || (homeDirectory ? path4.join(homeDirectory, ".cache") : void 0);
    exports2.runtime = env2.XDG_RUNTIME_DIR || void 0;
    exports2.dataDirs = (env2.XDG_DATA_DIRS || "/usr/local/share/:/usr/share/").split(":");
    if (exports2.data) {
      exports2.dataDirs.unshift(exports2.data);
    }
    exports2.configDirs = (env2.XDG_CONFIG_DIRS || "/etc/xdg").split(":");
    if (exports2.config) {
...
var require_write_file_atomic = __commonJS({
  "node_modules/.pnpm/write-file-atomic@3.0.3/node_modules/write-file-atomic/index.js"(exports2, module2) {
    ...
    function cleanupOnExit(tmpfile) {
      return () => {
        try {
          fs8.unlinkSync(typeof tmpfile === "function" ? tmpfile() : tmpfile);
        } catch (_) {
        }
      };
    }
    ...
    async function writeFileAsync(filename, data, options = {}) {
      ...
      try {
        ...
        const truename = await promisify6(fs8.realpath)(filename).catch(() => filename);
        tmpfile = getTmpname(truename);
        if (!options.mode || !options.chown) {
          const stats = await promisify6(fs8.stat)(truename).catch(() => {
          });
          if (stats) {
        ...
        fd = await promisify6(fs8.open)(tmpfile, "w", options.mode);
        ...
        if (Buffer.isBuffer(data)) {
          await promisify6(fs8.write)(fd, data, 0, data.length, 0);
        } else if (data != null) {
          await promisify6(fs8.write)(fd, String(data), 0, String(options.encoding || "utf8"));
        }
        if (options.fsync !== false) {
          await promisify6(fs8.fsync)(fd);
        }
        await promisify6(fs8.close)(fd);
        fd = null;
        if (options.chown) {
          await promisify6(fs8.chown)(tmpfile, options.chown.uid, options.chown.gid).catch((err) => {
            if (!isChownErrOk(err)) {
              throw err;
            }
          });
        ...
        if (options.mode) {
          await promisify6(fs8.chmod)(tmpfile, options.mode).catch((err) => {
            if (!isChownErrOk(err)) {
              throw err;
            }
          });
        }
        await promisify6(fs8.rename)(tmpfile, truename);
      } finally {
        if (fd) {
          await promisify6(fs8.close)(fd).catch(
            /* istanbul ignore next */
            () => {
            }
          );
        ...
        removeOnExitHandler();
        await promisify6(fs8.unlink)(tmpfile).catch(() => {
        });
    ...
    function writeFileSync(filename, data, options) {
      ...
      try {
        filename = fs8.realpathSync(filename);
      } catch (ex) {
      }
      const tmpfile = getTmpname(filename);
      if (!options.mode || !options.chown) {
        try {
          const stats = fs8.statSync(filename);
      ...
      let threw = true;
      try {
        fd = fs8.openSync(tmpfile, "w", options.mode || 438);
        if (options.tmpfileCreated) {
        ...
        if (Buffer.isBuffer(data)) {
          fs8.writeSync(fd, data, 0, data.length, 0);
        } else if (data != null) {
          fs8.writeSync(fd, String(data), 0, String(options.encoding || "utf8"));
        }
        if (options.fsync !== false) {
          fs8.fsyncSync(fd);
        }
        ...
        if (options.chown) {
          try {
            fs8.chownSync(tmpfile, options.chown.uid, options.chown.gid);
          } catch (err) {
            if (!isChownErrOk(err)) {
              throw err;
            }
        ...
        if (options.mode) {
          try {
            fs8.chmodSync(tmpfile, options.mode);
          } catch (err) {
            if (!isChownErrOk(err)) {
              throw err;
            }
        ...
        fs8.renameSync(tmpfile, filename);
        threw = false;
      } finally {
        if (fd) {
          try {
...
// node_modules/.pnpm/configstore@5.0.1/node_modules/configstore/index.js
var require_configstore = __commonJS({
  "node_modules/.pnpm/configstore@5.0.1/node_modules/configstore/index.js"(exports2, module2) {
    "use strict";
    var path4 = require("path");
    var os3 = require("os");
    ...
    var configDirectory = xdgBasedir.config || path4.join(os3.tmpdir(), uniqueString());
    var permissionError = "You don't have access to this file.";
    var makeDirOptions = { mode: 448 };
    var writeFileOptions = { mode: 384 };
    var Configstore2 = class {
      constructor(id, defaults2, options = {}) {
        const pathPrefix = options.globalConfigPath ? path4.join(id, "config.json") : path4.join("configstore", `${id}.json`);
        this.path = options.configPath || path4.join(configDirectory, pathPrefix);
        if (defaults2) {
          this.all = {
            ...defaults2,
            ...this.all
      ...
      get all() {
        try {
          return JSON.parse(fs8.readFileSync(this.path, "utf8"));
        } catch (error) {
          if (error.code === "ENOENT") {
            return {};
          }
...
`;
      ...
      set all(value) {
        try {
          makeDir.sync(path4.dirname(this.path), makeDirOptions);
...
var powerShellPath = async () => {
  ...
  return `${import_node_process3.default.env.SYSTEMROOT || import_node_process3.default.env.windir || String.raw`C:\Windows`}\\System32\\WindowsPowerShell\\v1.0\\powershell.exe`;
};
...
var import_configstore = __toESM(require_configstore());
var store = new import_configstore.default("ashgate-cli", {
...
// src/auth/keycloak.ts
var KEYCLOAK_URL = process.env.WALLET_KEYCLOAK_URL ?? "https://accounts.ashgateway.com";
var REALM = process.env.WALLET_KEYCLOAK_REALM ?? "ash";
var CLIENT_ID = process.env.WALLET_KEYCLOAK_CLIENT_ID ?? "ash-wallet-cli";
var CALLBACK_PORT = 7357;
var CALLBACK_URL = `http://localhost:${CALLBACK_PORT}/callback`;
function base64url(buf) {
  return buf.toString("base64").replace(/\+/g, "-").replace(/\//g, "_").replace(/=/g, "");
}
function generatePKCE() {
  const verifier = base64url(import_crypto2.default.randomBytes(32));
  const challenge = base64url(
    import_crypto2.default.createHash("sha256").update(verifier).digest()
  );
  return { verifier, challenge };
}
async function loginWithKeycloak() {
  const { verifier, challenge } = generatePKCE();
  const state = base64url(import_crypto2.default.randomBytes(16));
  const authUrl = new import_url2.URL(
  ...
  console.log(source_default.dim(`  Si rien ne s'ouvre, visitez:
  ${authUrl.toString()}
`));
  await open_default(authUrl.toString());
  const code = await new Promise((resolve, reject) => {
    const server = import_http4.default.createServer((req, res) => {
      const url2 = new import_url2.URL(req.url, `http://localhost:${CALLBACK_PORT}`);
      const returnedCode = url2.searchParams.get("code");
      const returnedState = url2.searchParams.get("state");
      if (returnedState !== state) {
        res.writeHead(200, { "Content-Type": "text/html; charset=utf-8" });
        res.end("<h2>\u274C State mismatch \u2014 s\xE9curit\xE9 compromise</h2>");
    ...
    <div style="text-align: center; font-family: sans-serif; margin-top: 50px;">
      <h1>\u2705 Authentification r\xE9ussie !</h1>
      <p>Vous pouvez fermer cette fen\xEAtre et revenir au terminal.</p>
    </div>
  `);
      clearTimeout(timeout);
      server.close();
  ...
  const response = await axios_default.post(tokenUrl, params.toString(), {
    headers: { "Content-Type": "application/x-www-form-urlencoded" },
    // keepAlive: false → le process quitte naturellement après la requête
    httpAgent: new import_http4.default.Agent({ keepAlive: false }),
    httpsAgent: new import_https2.default.Agent({ keepAlive: false })
  });
  ...
  const tokenStore = {
    ...
    refreshExpiresAt: Date.now() + (refresh_expires_in > 0 ? refresh_expires_in : 259200) * 1e3,
    keycloakId: payload.sub,
    email: payload.email
  };
  walletConfig.setTokens(tokenStore);
  console.log(source_default.green(`
\u2713 Connect\xE9 en tant que ${payload.email ?? payload.sub}`));
}
async function refreshTokenIfNeeded() {
  const tokens = walletConfig.getTokens();
  if (!tokens.refreshToken) return;
  if (tokens.expiresAt > Date.now() + 6e4) return;
  const tokenUrl = `${KEYCLOAK_URL}/realms/${REALM}/protocol/openid-connect/token`;
  const params = new URLSearchParams({
  ...
  try {
    ...
    throw new Error("Session expir\xE9e. Veuillez vous reconnecter : ashgate auth login");
  }
}
function requireAuth() {
  if (!walletConfig.isAuthenticated()) {
    console.error(source_default.red("\u2717 Non authentifi\xE9. Lancez : ashgate auth login"));
    process.exit(1);
  }
}
...
function registerAuthCommands(program3) {
  const auth = program3.command("auth").description("Gestion de l'authentification");
  auth.command("login").description("Se connecter \xE0 la plateforme Ashgate via Keycloak").action(async () => {
    try {
      await loginWithKeycloak();
    } catch (err) {
      console.error(source_default.red("\u2717 \xC9chec de la connexion :"), err.message);
      process.exit(1);
    }
  });
  auth.command("logout").description("Se d\xE9connecter (supprime les tokens locaux)").action(() => {
    walletConfig.clearTokens();
    console.log(source_default.green("\u2713 D\xE9connect\xE9. \xC0 bient\xF4t !"));
  });
  auth.command("status").description("Afficher l'\xE9tat de la session courante").action(() => {
    const tokens = walletConfig.getTokens();
    if (!tokens.refreshToken || tokens.refreshExpiresAt <= Date.now()) {
      console.log(source_default.yellow("\u2717 Non authentifi\xE9. Lancez : ashgate auth login"));
      return;
    }
    ...
    const minutesLeft = Math.floor(msLeft % 36e5 / 6e4);
    const expiryStr = daysLeft > 0 ? `${daysLeft}j ${hoursLeft}h` : hoursLeft > 0 ? `${hoursLeft}h ${minutesLeft}min` : `${minutesLeft}min`;
    console.log(source_default.green(`\u2713 Connect\xE9`));
...
apiClient.interceptors.request.use(
  async (config) => {
    ...
    try {
      ...
      console.error("\nSession expir\xE9e ou invalide. Veuillez vous reconnecter.");
      process.exit(1);
    }
    const tokens = walletConfig.getTokens();
...
function mapOptionsToDto(options) {
  ...
  const valOrUndefined = (val1, val2) => {
    if (val1 !== void 0) return val1;
    if (val2 !== void 0) return val2;
    return void 0;
  };
  const sandboxVal = valOrUndefined(sandboxApiKey, sandboxKey);
  if (sandboxVal !== void 0) dto.sandboxApiKey = sandboxVal;
  const liveVal = valOrUndefined(liveApiKey, liveKey);
  if (liveVal !== void 0) dto.liveApiKey = liveVal;
  const feexpayVal = valOrUndefined(feexpayApiKey, feexpayKey);
  if (feexpayVal !== void 0) dto.feexpayApiKey = feexpayVal;
  const feexpayShopVal = valOrUndefined(feexpayShopId, feexpayShop);
  if (feexpayShopVal !== void 0) dto.feexpayShopId = feexpayShopVal;
  const stripeKeyVal = valOrUndefined(stripeSecretKey, stripeKey);
  if (stripeKeyVal !== void 0) dto.stripeSecretKey = stripeKeyVal;
  const stripePubVal = valOrUndefined(stripePublishableKey, stripePub);
  if (stripePubVal !== void 0) dto.stripePublishableKey = stripePubVal;
  const stripeWebhookVal = valOrUndefined(stripeWebhookSecret, stripeWebhook);
  if (stripeWebhookVal !== void 0) dto.stripeWebhookSecret = stripeWebhookVal;
  return dto;
}
function registerProjectCommands(program3) {
  const project = program3.command("project").description("Gestion des projets de paiement");
  project.command("list").description("Lister tous vos projets").action(async () => {
    requireAuth();
    try {
      const response = await apiClient.get("/projects");
      const projects = response.data;
      if (!Array.isArray(projects) || projects.length === 0) {
        console.log(source_default.yellow("\n\u2139 Aucun projet trouv\xE9. Cr\xE9ez-en un avec : ashgate project create"));
        return;
      }
      ...
      for (const p of projects) {
        const statusStr = p.isActive !== false ? source_default.green("actif") : source_default.red("inactif");
        console.log(
          `${(p.name || "").padEnd(25)} ${(p.slug || "").padEnd(25)} ${(p.billingMode || "none").padEnd(15)} ${statusStr}`
        );
      }
      console.log("");
    } catch (err) {
      console.error(source_default.red("\u2717 Erreur lors de la r\xE9cup\xE9ration des projets :"), err.response?.data?.message || err.message);
      process.exit(1);
    }
  });
  project.command("create").description("Cr\xE9er un nouveau projet").requiredOption("-n, --name <name>", "Nom du projet").requiredOption("-s, --slug <slug>", "Slug unique (URL-safe)").option("-b, --billing-mode <billingMode>", "Mode de facturation (none, subscription, commission)", "none").option("-c, --commission-rate <rate>", "Taux de commission (%)", parseFloat).option("-w, --webhook-url <webhookUrl>", "URL de notification (webhook) de destination").option("--sandbox-key <sandboxApiKey>", "Cl\xE9 API FedaPay Sandbox").option("--live-key <liveApiKey>", "Cl\xE9 API FedaPay Live").option("--feexpay-key <feexpayApiKey>", "Cl\xE9 API Feexpay").option("--feexpay-shop <feexpayShopId>", "ID Boutique Feexpay").option("--webhook-secret <webhookSecret>", "Secret du Webhook FedaPay").option("--stripe-key <stripeSecretKey>", "Cl\xE9 API Stripe Secr\xE8te (Secret Key)").option("--stripe-pub <stripePublishableKey>", "Cl\xE9 API Stripe Publique (Publishable Key)").option("--stripe-webhook <stripeWebhookSecret>", "Secret du Webhook Stripe").option("--send-invoices <sendInvoices>", "Envoyer les factures par email automatiquement (true/false)", (v) => v === "true").action(async (options) => {
    requireAuth();
    try {
      const dto = mapOptionsToDto(options);
      const response = await apiClient.post("/projects", dto);
      console.log(source_default.green(`
\u2713 Projet cr\xE9\xE9 avec succ\xE8s !`));
      console.log(`  Nom  : ${response.data.name}`);
      console.log(`  Slug : ${response.data.slug}`);
      console.log(`  Cl\xE9 Publique  : ${response.data.publicKey}`);
      console.log(`  Cl\xE9 Secr\xE8te   : ${response.data.secretKey}
`);
    } catch (err) {
      console.error(source_default.red("\u2717 \xC9chec de la cr\xE9ation du projet :"), err.response?.data?.message || err.message);
      process.exit(1);
    }
  });
  project.command("show <slug>").description("Afficher les d\xE9tails d'un projet").action(async (slug) => {
    requireAuth();
    try {
      const response = await apiClient.get(`/projects/${slug}`);
      const p = response.data;
      console.log(source_default.bold(`
D\xE9tails du projet [${p.name}] :`));
      ...
      console.log(`  Statut        : ${p.isActive !== false ? source_default.green("Actif") : source_default.red("Inactif")}`);
      console.log(`  Webhook URL   : ${p.webhookUrl || source_default.dim("non configur\xE9")}`);
      console.log(`  Priorit\xE9 Pmt  : ${p.paymentProviderPriority?.join(" -> ") || "non configur\xE9"}`);
      console.log(`  Cl\xE9 Publique  : ${p.publicKey || "non configur\xE9e"}`);
      console.log(`  Cl\xE9 Secr\xE8te   : ${p.secretKey || "non configur\xE9e"}`);
      console.log(source_default.bold("\nConfiguration des API keys :"));
      console.log(`  FedaPay Sandbox API Key : ${p.sandboxApiKey ? source_default.green("Configur\xE9e") : source_default.dim("Non configur\xE9e")}`);
      console.log(`  FedaPay Live API Key    : ${p.liveApiKey ? source_default.green("Configur\xE9e") : source_default.dim("Non configur\xE9e")}`);
      console.log(`  FedaPay Webhook Secret  : ${p.webhookSecret ? source_default.green("Configur\xE9e") : source_default.dim("Non configur\xE9e")}`);
      console.log(`  FeexPay API Key         : ${p.feexpayApiKey ? source_default.green("Configur\xE9e") : source_default.dim("Non configur\xE9e")}`);
      console.log(`  FeexPay Shop ID         : ${p.feexpayShopId ? source_default.green("Configur\xE9") : source_default.dim("Non configur\xE9")}`);
      console.log(`  Stripe Secret Key       : ${p.stripeSecretKey ? source_default.green("Configur\xE9e") : source_default.dim("Non configur\xE9e")}`);
      console.log(`  Stripe Publishable Key  : ${p.stripePublishableKey ? source_default.green("Configur\xE9e") : source_default.dim("Non configur\xE9e")}`);
      console.log(`  Stripe Webhook Secret   : ${p.stripeWebhookSecret ? source_default.green("Configur\xE9") : source_default.dim("Non configur\xE9")}
`);
    } catch (err) {
      console.error(source_default.red("\u2717 Impossible d'afficher le projet :"), err.response?.data?.message || err.message);
      process.exit(1);
    }
  });
  project.command("update <slug>").description("Mettre \xE0 jour la configuration d'un projet").option("-n, --name <name>", "Nom du projet").option("-b, --billing-mode <billingMode>", "Mode de facturation (none, subscription, commission)").option("-c, --commission-rate <rate>", "Taux de commission (%)", parseFloat).option("-w, --webhook-url <webhookUrl>", "URL de notification (webhook)").option("--sandbox-key <sandboxApiKey>", "Cl\xE9 API FedaPay Sandbox").option("--live-key <liveApiKey>", "Cl\xE9 API FedaPay Live").option("--feexpay-key <feexpayApiKey>", "Cl\xE9 API Feexpay").option("--feexpay-shop <feexpayShopId>", "ID Boutique Feexpay").option("--webhook-secret <webhookSecret>", "Secret du Webhook FedaPay").option("--stripe-key <stripeSecretKey>", "Cl\xE9 API Stripe Secr\xE8te (Secret Key)").option("--stripe-pub <stripePublishableKey>", "Cl\xE9 API Stripe Publique (Publishable Key)").option("--stripe-webhook <stripeWebhookSecret>", "Secret du Webhook Stripe").option("--send-invoices <sendInvoices>", "Envoyer les factures (true/false)", (v) => v === "true").action(async (slug, options) => {
    requireAuth();
    try {
      const dto = mapOptionsToDto(options);
      const body = Object.fromEntries(Object.entries(dto).filter(([_, v]) => v !== void 0));
      const response = await apiClient.patch(`/projects/${slug}`, body);
      console.log(source_default.green(`
\u2713 Projet "${response.data.name}" mis \xE0 jour avec succ\xE8s !
`));
    } catch (err) {
      console.error(source_default.red("\u2717 \xC9chec de la mise \xE0 jour :"), err.response?.data?.message || err.message);
      process.exit(1);
    }
  });
  project.command("delete <slug>").description("Supprimer (d\xE9sactiver) un projet").action(async (slug) => {
    requireAuth();
    try {
      await apiClient.delete(`/projects/${slug}`);
      console.log(source_default.green(`
\u2713 Projet "${slug}" d\xE9sactiv\xE9/supprim\xE9 avec succ\xE8s.
`));
    } catch (err) {
      console.error(source_default.red("\u2717 \xC9chec de la suppression :"), err.response?.data?.message || err.message);
      process.exit(1);
    }
  });
  project.command("usage <slug>").description("Afficher les quotas et l'utilisation du mois en cours").action(async (slug) => {
    requireAuth();
    try {
...
Quotas et utilisation pour "${slug}" (${usage.month}) :`));
      console.log(`  Plan SaaS Actif : ${source_default.blue(usage.planName)}`);
...
`);
      ...
      console.error(source_default.red("\u2717 Erreur lors de la r\xE9cup\xE9ration des quotas :"), err.response?.data?.message || err.message);
      process.exit(1);
    }
  });
  project.command("logs <slug>").description("Streamer les logs de requ\xEAtes du proxy en temps r\xE9el").action(async (slug) => {
    requireAuth();
    console.log(source_default.cyan(`
...
`));
    try {
          ...
          for (const line of lines) {
            if (line.startsWith("data: ")) {
              try {
                ...
                const statusColor = logObj.status >= 500 ? source_default.red : logObj.status >= 400 ? source_default.yellow : source_default.green;
                const methodColor = logObj.method === "POST" ? source_default.magenta : logObj.method === "DELETE" ? source_default.red : logObj.method === "PATCH" ? source_default.blue : source_default.cyan;
                console.log(
                  `[${source_default.dim(logObj.timestamp)}] ${methodColor(logObj.method.padEnd(6))} ${source_default.white(logObj.path.padEnd(30))} Status: ${statusColor(logObj.status)} Duration: ${source_default.yellow(logObj.duration + "ms")} Env: ${source_default.blue(logObj.env)}`
                );
              } catch (e) {
              }
            }
      ...
      stream4.on("error", (err) => {
        console.error(source_default.red("\u2717 Erreur de flux :"), err.message);
        process.exit(1);
      });
      stream4.on("end", () => {
        console.log(source_default.yellow("\n\u2139 Flux de logs ferm\xE9 par le serveur.\n"));
      });
    } catch (err) {
      console.error(source_default.red("\u2717 Impossible d'\xE9tablir la connexion de streaming :"), err.response?.data?.message || err.message);
      process.exit(1);
    }
  });
}
...
function registerCompletionCommands(program3) {
  program3.command("completion <shell>").description("G\xE9n\xE9rer les scripts d'autocompl\xE9tion pour votre shell (bash ou zsh)").action((shell) => {
    const shellNormalized = shell.toLowerCase();
    if (shellNormalized === "bash") {
      console.log(`
# Script d'autocompl\xE9tion Bash pour ashgate-cli
# Pour l'activer, ajoutez la ligne suivante \xE0 votre ~/.bashrc :
...
_ashgate_completion() {
  ...
  # Passer tous les mots saisis apr\xE8s 'ashgate' jusqu'au curseur actuel
...
`);
      ...
      console.log(`
# Script d'autocompl\xE9tion Zsh pour ashgate-cli
# Pour l'activer, ajoutez la ligne suivante \xE0 votre ~/.zshrc :
...
_ashgate_completion() {
  ...
  # R\xE9cup\xE9rer les mots saisis
  ...
  # S\xE9parer par retour \xE0 la ligne
...
`);
      ...
      console.error(source_default.red(`\u2717 Shell non support\xE9 : "${shell}". Les shells support\xE9s sont "bash" et "zsh".`));
      process.exit(1);
    }
  });
  program3.command("__complete", { hidden: true }).argument("[words...]").action(async (words = []) => {
...
var import_promises3 = __toESM(require("readline/promises"));
var import_process = require("process");
var import_child_process = require("child_process");
var rlInstance = null;
function getReadlineInterface() {
  if (!rlInstance) {
    rlInstance = import_promises3.default.createInterface({ input: import_process.stdin, output: import_process.stdout });
  }
  return rlInstance;
}
...
function registerInitCommands(program3) {
  program3.command("init").description("D\xE9tecter le projet local, configurer les cl\xE9s et installer les composants de paiement").action(async () => {
    try {
      console.log(source_default.bold.cyan("\n\u{1F680} Initialisation d'Ash Gateway dans votre projet local..."));
      const cwd = process.cwd();
      let detectedType = null;
      let projectPath = cwd;
      if (import_fs.default.existsSync(import_path.default.join(cwd, "pubspec.yaml"))) {
        detectedType = "flutter";
      } else if (import_fs.default.existsSync(import_path.default.join(cwd, "Gemfile"))) {
        detectedType = "rails";
      } else if (import_fs.default.existsSync(import_path.default.join(cwd, "package.json"))) {
        try {
          const pkg = JSON.parse(import_fs.default.readFileSync(import_path.default.join(cwd, "package.json"), "utf8"));
      ...
      if (!detectedType) {
        console.log(source_default.yellow("\nAucun projet compatible d\xE9tect\xE9 directement dans le dossier actuel."));
        const searchDirs = import_fs.default.readdirSync(cwd).filter((f) => import_fs.default.statSync(import_path.default.join(cwd, f)).isDirectory());
        for (const dir of searchDirs) {
          const subPath = import_path.default.join(cwd, dir);
          if (import_fs.default.existsSync(import_path.default.join(subPath, "pubspec.yaml"))) {
            detectedType = "flutter";
            projectPath = subPath;
            break;
          } else if (import_fs.default.existsSync(import_path.default.join(subPath, "package.json"))) {
            try {
              const pkg = JSON.parse(import_fs.default.readFileSync(import_path.default.join(subPath, "package.json"), "utf8"));
              const deps = { ...pkg.dependencies, ...pkg.devDependencies };
              if (deps.nuxt) {
                detectedType = "nuxt";
                projectPath = subPath;
      ...
      if (detectedType) {
        console.log(source_default.green(`\u2713 Projet d\xE9tect\xE9 : ${source_default.bold(detectedType.toUpperCase())} dans ${import_path.default.relative(cwd, projectPath) || "."}`));
      } else {
        console.log(source_default.cyan("\nTypes de projets support\xE9s : flutter, nuxt, vue, next, react, express, rails"));
        ...
        if (["flutter", "nuxt", "vue", "next", "react", "express", "rails"].includes(type)) {
          ...
          console.error(source_default.red("\u2717 Type de projet non support\xE9."));
          process.exit(1);
        }
      }
      let projectKey = "";
      ...
      if (!walletConfig.isAuthenticated()) {
        console.log(source_default.yellow("\nVous n'\xEAtes pas connect\xE9. Connexion requise pour continuer."));
        try {
          await loginWithKeycloak();
        } catch (err) {
          console.error(source_default.red(`
\u2717 \xC9chec de la connexion : ${err.message}`));
          process.exit(1);
        }
      }
      if (walletConfig.isAuthenticated()) {
        try {
          ...
          if (Array.isArray(projects) && projects.length > 0) {
            ...
            if (index >= 0 && index < projects.length) {
              ...
              console.log(source_default.green(`\u2713 Projet s\xE9lectionn\xE9 : ${projects[index].name}`));
            }
          } else {
            console.log(source_default.yellow("\nAucun projet trouv\xE9 sur votre compte."));
          }
        } catch (err) {
          console.log(source_default.yellow(`
\u26A0\uFE0F  Impossible de charger les projets depuis l'API (${err.message}).`));
      ...
      if (!projectKey) {
        console.log(source_default.yellow("\n(Configuration manuelle des cl\xE9s)"));
        projectKey = await askQuestion("Entrez la cl\xE9 publique de votre projet (ap_pub_xxx) : ");
        projectSlug = await askQuestion("Entrez le slug de votre projet : ");
      }
      const envSelection = await askQuestion("\nChoisissez l'environnement (1. sandbox [d\xE9faut], 2. live) : ");
      if (envSelection === "2") {
        environment = "live";
      }
      ...
      console.log("  5. Tous (FedaPay, FeexPay et Stripe) [d\xE9faut]");
      ...
      let feexpayShopId = "";
      if (useFeexpay) {
        console.log(source_default.cyan("\nMode d'int\xE9gration pour FeexPay :"));
        console.log("  1. Proxy/Serveur USSD (Recommand\xE9 - S\xE9curis\xE9 et sans SDK local) [d\xE9faut]");
        console.log("  2. SDK local (feexpay_flutter - n\xE9cessite d'exposer vos cl\xE9s)");
        const modeSelection = await askQuestion("Choisissez une option (1-2) : ");
        if (modeSelection === "2") {
          feexpayMode = "sdk";
          console.log(source_default.yellow("\n(Configuration des cl\xE9s FeexPay requise pour le SDK local)"));
          feexpayToken = await askQuestion("Entrez votre cl\xE9 API / Token FeexPay (ex: fp_xxxx ou Bearer token) : ");
          feexpayShopId = await askQuestion("Entrez votre Shop ID FeexPay : ");
        }
      }
      try {
        if (detectedType === "flutter") {
          const pubspecPath = import_path.default.join(projectPath, "pubspec.yaml");
          const pubspecContent = import_fs.default.readFileSync(pubspecPath, "utf8");
          if (useFedapay) {
            if (!pubspecContent.includes("feda_flutter:")) {
              console.log(source_default.cyan("\nInstallation de la d\xE9pendance feda_flutter..."));
              try {
                (0, import_child_process.execSync)("flutter pub add feda_flutter", { cwd: projectPath, stdio: "inherit" });
                console.log(source_default.green("\u2713 D\xE9pendance feda_flutter ajout\xE9e avec succ\xE8s."));
              } catch (err) {
                console.warn(source_default.yellow("\u26A0\uFE0F  Impossible d'ajouter feda_flutter via la CLI flutter. Veuillez l'ajouter manuellement \xE0 vos dependencies dans pubspec.yaml."));
              }
            }
          }
          if (useFeexpay && feexpayMode === "sdk") {
            if (!pubspecContent.includes("feexpay_flutter:")) {
              console.log(source_default.cyan("\nInstallation de la d\xE9pendance feexpay_flutter..."));
              try {
                (0, import_child_process.execSync)("flutter pub add feexpay_flutter", { cwd: projectPath, stdio: "inherit" });
                console.log(source_default.green("\u2713 D\xE9pendance feexpay_flutter ajout\xE9e avec succ\xE8s."));
              } catch (err) {
                console.warn(source_default.yellow("\u26A0\uFE0F  Impossible d'ajouter feexpay_flutter via la CLI flutter. Veuillez l'ajouter manuellement \xE0 vos dependencies dans pubspec.yaml."));
              }
            }
          }
          if (useFeexpay && feexpayMode === "proxy" || useFedapay && useFeexpay || useStripe) {
            if (!pubspecContent.includes("webview_flutter:")) {
              console.log(source_default.cyan("\nInstallation de la d\xE9pendance webview_flutter..."));
              try {
                (0, import_child_process.execSync)("flutter pub add webview_flutter", { cwd: projectPath, stdio: "inherit" });
                console.log(source_default.green("\u2713 D\xE9pendance webview_flutter ajout\xE9e avec succ\xE8s."));
              } catch (err) {
                console.warn(source_default.yellow("\u26A0\uFE0F  Impossible d'ajouter webview_flutter via la CLI flutter. Veuillez l'ajouter manuellement \xE0 vos dependencies dans pubspec.yaml."));
              }
            }
          }
          const libDir = import_path.default.join(projectPath, "lib");
          const providersDir = import_path.default.join(libDir, "providers");
          if (!import_fs.default.existsSync(providersDir)) {
            import_fs.default.mkdirSync(providersDir, { recursive: true });
          }
          ...
          const configContent = `// G\xE9n\xE9r\xE9 automatiquement par ashgate init
${fedaImport}
class AshgateConfig {
...
`;
          import_fs.default.writeFileSync(import_path.default.join(libDir, "ashgate_config.dart"), configContent);
          console.log(source_default.green("\u2713 Fichier lib/ashgate_config.dart g\xE9n\xE9r\xE9."));
          const providerBaseContent = `// G\xE9n\xE9r\xE9 automatiquement par ashgate init
import 'package:flutter/material.dart';

/// Mod\xE8le unifi\xE9 de demande de paiement pour Ash Gateway.
...
/// R\xE9sultat unifi\xE9 renvoy\xE9 apr\xE8s l'initiation d'un paiement.
...
/// Interface commune pour toutes les passerelles de paiement de l'\xE9cosyst\xE8me.
abstract class AshgatePaymentProvider {
  Future<AshgatePaymentResult> pay(AshgatePaymentRequest request);
}
`;
          import_fs.default.writeFileSync(import_path.default.join(libDir, "ashgate_payment_provider.dart"), providerBaseContent);
          console.log(source_default.green("\u2713 Fichier lib/ashgate_payment_provider.dart g\xE9n\xE9r\xE9."));
          if (useFedapay) {
            const fedapayProviderContent = `// G\xE9n\xE9r\xE9 automatiquement par ashgate init
...
class FedapayProvider implements AshgatePaymentProvider {
  ...
  Future<AshgatePaymentResult> pay(AshgatePaymentRequest request) async {
    try {
      // 1. Adapter la requ\xEAte plate vers le mod\xE8le FedaPay
      ...
      // 2. Cr\xE9er la transaction via le SDK feda_flutter
      ...
      if (transactionId == null) {
        return AshgatePaymentResult(success: false, errorMessage: "Erreur lors de la cr\xE9ation de la transaction FedaPay.");
      }

      // 3. R\xE9cup\xE9rer le token de paiement
...
`;
            import_fs.default.writeFileSync(import_path.default.join(providersDir, "fedapay_provider.dart"), fedapayProviderContent);
            console.log(source_default.green("\u2713 Fichier lib/providers/fedapay_provider.dart g\xE9n\xE9r\xE9."));
          } else {
            const oldFeda = import_path.default.join(providersDir, "fedapay_provider.dart");
            if (import_fs.default.existsSync(oldFeda)) import_fs.default.unlinkSync(oldFeda);
          }
          if (useStripe) {
            const stripeProviderContent = `// G\xE9n\xE9r\xE9 automatiquement par ashgate init
import 'dart:convert';
import 'dart:io';
...
`;
            import_fs.default.writeFileSync(import_path.default.join(providersDir, "stripe_provider.dart"), stripeProviderContent);
            console.log(source_default.green("\u2713 Fichier lib/providers/stripe_provider.dart g\xE9n\xE9r\xE9."));
          } else {
            const oldStripe = import_path.default.join(providersDir, "stripe_provider.dart");
            if (import_fs.default.existsSync(oldStripe)) import_fs.default.unlinkSync(oldStripe);
          }
          if (useFeexpay) {
            let feexpayProviderContent = "";
            if (feexpayMode === "sdk") {
              feexpayProviderContent = `// G\xE9n\xE9r\xE9 automatiquement par ashgate init
import 'package:flutter/material.dart';
...
class FeexpayProvider implements AshgatePaymentProvider {
  ...
  Future<AshgatePaymentResult> pay(AshgatePaymentRequest request) async {
    final client = HttpClient();
    try {
      // R\xE9soudre le r\xE9seau compatible FeexPay
...
`;
            ...
            import_fs.default.writeFileSync(import_path.default.join(providersDir, "feexpay_provider.dart"), feexpayProviderContent);
            console.log(source_default.green("\u2713 Fichier lib/providers/feexpay_provider.dart g\xE9n\xE9r\xE9."));
          } else {
            const oldFeex = import_path.default.join(providersDir, "feexpay_provider.dart");
            if (import_fs.default.existsSync(oldFeex)) import_fs.default.unlinkSync(oldFeex);
          }
          const imports = [
            "import 'package:flutter/material.dart';"
          ];
          ...
          providerResolver += `
    throw Exception("Le fournisseur de paiement '$providerName' n'est pas support\xE9.");`;
          let payWidgetHelper = "";
          if (useFedapay && (useFeexpay && feexpayMode === "proxy" || useStripe)) {
            payWidgetHelper = `
  /// Widget de paiement unifi\xE9 (FedaPay, FeexPay ou Stripe)
  static Widget payWidget({
    ...
    return const SizedBox();
            ...
            payWidgetHelper = `
  /// Widget de paiement unifi\xE9 (FedaPay)
  static Widget payWidget({
    String? transactionToken,
    String? paymentUrl,
    required VoidCallback onPaymentSuccess,
    ...
    return const SizedBox();
            ...
            payWidgetHelper = `
  /// Widget de paiement unifi\xE9 (WebView)
  static Widget payWidget({
    String? transactionToken,
    String? paymentUrl,
    required VoidCallback onPaymentSuccess,
    ...
    return const SizedBox();
            ...
            payWidgetHelper = `
  /// Widget de paiement unifi\xE9 (Non configur\xE9 pour ce mode)
  static Widget payWidget({
    String? transactionToken,
    String? paymentUrl,
    required VoidCallback onPaymentSuccess,
  ...
  /// Affiche une bo\xEEte de dialogue bottom sheet avec un WebView pour n'importe quelle URL de paiement (ex: FeexPay)
      ...
      builder: (sheetContext) => Container(
        ...
        child: ClipRRect(
          ...
          child: Column(
            children: [
              AppBar(
                title: const Text('Paiement S\xE9curis\xE9'),
                centerTitle: true,
                leading: IconButton(
            ...
            showPaymentSheetHelper = `
  /// Affiche une bo\xEEte de dialogue bottom sheet (Non configur\xE9 pour ce mode)
  static Future<void> showPaymentSheet({
    required BuildContext context,
    required String paymentUrl,
    required VoidCallback onPaymentSuccess,
...
`;
          ...
          if (useFeexpay && feexpayMode === "proxy" || useStripe) {
            webviewClass = `
/// WebView personnalis\xE9 pour \xE9couter la redirection de succ\xE8s / \xE9chec d'Ash Gateway
...
`;
  ...
  /// Lance la proc\xE9dure de paiement de mani\xE8re unifi\xE9e pour tous les modes et fournisseurs.
  static Future<AshgatePaymentResult> startPayment({
    ...
    return result;
          ...
          const orchestratorContent = `// G\xE9n\xE9r\xE9 automatiquement par ashgate init
${imports.join("\n")}
...
class AshgatePaymentService {
  ...
  /// R\xE9sout l'adaptateur de paiement correspondant
  AshgatePaymentProvider getProvider(String providerName) {${providerResolver}
  }

  /// D\xE9clenche le paiement sur le provider de votre choix
...
${webviewClass}
`;
          import_fs.default.writeFileSync(import_path.default.join(libDir, "ashgate_payment.dart"), orchestratorContent);
          console.log(source_default.green("\u2713 Fichier lib/ashgate_payment.dart g\xE9n\xE9r\xE9."));
          const mainDartPath = import_path.default.join(projectPath, "lib/main.dart");
          if (import_fs.default.existsSync(mainDartPath)) {
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const MyHomePage(title: '\u{1F680} Ashgate Checkout'),
    );
  }
}
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<void> _startCheckout() async {
    ...
    // D\xE9finir le montant et la devise selon le fournisseur
    double amount = _selectedProvider == 'stripe' ? 9.99 : 6500.0;
    // Si Stripe, le montant transmis doit \xEAtre en centimes (subunit) : 999
    ...
    try {
      final result = await AshgatePayment.startPayment(
        ...
        onPaymentSuccess: () {
          _showStatusDialog(
            title: 'Paiement R\xE9ussi !',
            message: 'Votre abonnement Premium est maintenant activ\xE9.',
            isSuccess: true,
          );
        },
        onPaymentFailed: () {
          _showStatusDialog(
            title: 'Annul\xE9 ou \xC9chou\xE9',
            message: 'La transaction a \xE9t\xE9 annul\xE9e ou a \xE9chou\xE9.',
            isSuccess: false,
          );
        },
      );
      ...
      if (result.success && _selectedProvider == 'fedapay') {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Veuillez confirmer le prompt de paiement sur votre t\xE9l\xE9phone...'),
      ...
      body: SingleChildScrollView(
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              // R\xE9capitulatif d'Achat Premium
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        const Text('Total \xE0 payer :', style: TextStyle(color: Colors.white70, fontSize: 16)),
                        Text(
              ...
              // Titre S\xE9lection du Fournisseur
              const Text(
                'Moyen de paiement',
              ...
              // Champs Utilisateur
              _buildInputLabel('Pr\xE9nom'),
              _buildTextField(_firstnameController, 'Pr\xE9nom', Icons.person_outline),
              ...
              // Champ T\xE9l\xE9phone + Op\xE9rateur (Affich\xE9s uniquement pour FedaPay/FeexPay)
              if (!isStripe) ...[
                _buildInputLabel('Num\xE9ro de T\xE9l\xE9phone'),
                _buildTextField(_phoneController, 'T\xE9l\xE9phone', Icons.phone, keyboardType: TextInputType.phone),
                const SizedBox(height: 16),

                _buildInputLabel('Op\xE9rateur mobile'),
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          "Vous allez \xEAtre redirig\xE9 vers l'interface s\xE9curis\xE9e de Stripe Checkout pour finaliser le paiement par carte bancaire.",
...
`;
              import_fs.default.writeFileSync(mainDartPath, mainDartDemoContent);
              console.log(source_default.green("\u2713 Fichier lib/main.dart remplac\xE9 par l'exemple de checkout fonctionnel."));
            } else {
              let mainContent = import_fs.default.readFileSync(mainDartPath, "utf8");
...
` + mainContent;
  ...
  ${runAppCall};
              ...
              import_fs.default.writeFileSync(mainDartPath, mainContent);
              console.log(source_default.green("\u2713 Fichier lib/main.dart mis \xE0 jour avec l'initialisation d'Ashgate."));
            }
          }
        } else if (detectedType === "nuxt") {
          const envPath = import_path.default.join(projectPath, ".env");
          const envVars = `
NUXT_PUBLIC_ASHGATE_API_URL=${cloudUrl}
NUXT_PUBLIC_ASHGATE_PROJECT_KEY=${projectKey}
NUXT_PUBLIC_ASHGATE_ENV=${environment}
`;
          import_fs.default.appendFileSync(envPath, envVars);
          console.log(source_default.green("\u2713 Fichier .env mis \xE0 jour avec les variables NUXT_PUBLIC_ASHGATE."));
          const serverApiDir = import_path.default.join(projectPath, "server", "api", "ashgate");
          if (!import_fs.default.existsSync(serverApiDir)) import_fs.default.mkdirSync(serverApiDir, { recursive: true });
  ...
  try {
    ...
    throw createError({
      ...
      statusMessage: err.data?.message || err.message || '\xC9chec du paiement Ashgate',
    });
  }
});
`;
          import_fs.default.writeFileSync(import_path.default.join(serverApiDir, "checkout.post.ts"), serverNitroContent);
          console.log(source_default.green("\u2713 Route serveur server/api/ashgate/checkout.post.ts g\xE9n\xE9r\xE9e."));
          const composablesDir = import_path.default.join(projectPath, "composables");
          if (!import_fs.default.existsSync(composablesDir)) import_fs.default.mkdirSync(composablesDir, { recursive: true });
  ...
  const initCheckout = async (params: {
    ...
    try {
      ...
      error.value = err.statusMessage || err.message || '\xC9chec du paiement';
      throw err;
    } finally {
...
`;
          import_fs.default.writeFileSync(import_path.default.join(composablesDir, "useAshgatePayment.ts"), composableContent);
          console.log(source_default.green("\u2713 Composable composables/useAshgatePayment.ts g\xE9n\xE9r\xE9."));
          const compDir = import_path.default.join(projectPath, "components");
          if (!import_fs.default.existsSync(compDir)) import_fs.default.mkdirSync(compDir, { recursive: true });
  ...
  <div class="ashgate-checkout p-6 bg-slate-900 text-white rounded-xl shadow-xl max-w-md mx-auto border border-slate-800">
    <h2 class="text-xl font-bold mb-4 text-center">Paiement S\xE9curis\xE9 Ashgate</h2>
    ...
    <form @submit.prevent="handlePay" class="space-y-4">
      ...
      <div v-if="form.provider !== 'stripe'">
        <label class="block text-sm font-medium mb-1 text-slate-300">T\xE9l\xE9phone Mobile Money</label>
        <input
      ...
      <div v-if="form.provider !== 'stripe'">
        <label class="block text-sm font-medium mb-1 text-slate-300">Op\xE9rateur</label>
        <select
...
`;
          import_fs.default.writeFileSync(import_path.default.join(compDir, "AshgateCheckout.vue"), vueContent);
          console.log(source_default.green("\u2713 Composant components/AshgateCheckout.vue g\xE9n\xE9r\xE9."));
...
`;
          import_fs.default.appendFileSync(envPath, envVars);
          console.log(source_default.green("\u2713 Fichier .env mis \xE0 jour avec les variables VITE_ASHGATE."));
          const srcDir = import_path.default.join(projectPath, "src");
          const compDir = import_path.default.join(srcDir, "components");
          const composablesDir = import_path.default.join(srcDir, "composables");
          if (!import_fs.default.existsSync(compDir)) import_fs.default.mkdirSync(compDir, { recursive: true });
          if (!import_fs.default.existsSync(composablesDir)) import_fs.default.mkdirSync(composablesDir, { recursive: true });
...
export function useAshgatePayment() {
  ...
  const initCheckout = async (params) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.message || '\xC9chec de l\u2019initialisation');
...
`;
          import_fs.default.writeFileSync(import_path.default.join(composablesDir, "useAshgatePayment.js"), composableVue);
          console.log(source_default.green("\u2713 Composable src/composables/useAshgatePayment.js g\xE9n\xE9r\xE9."));
...
`;
          import_fs.default.appendFileSync(envPath, envVars);
          console.log(source_default.green("\u2713 Fichier .env mis \xE0 jour avec NEXT_PUBLIC_ASHGATE."));
          const appApiDir = import_path.default.join(projectPath, "app", "api", "ashgate", "checkout");
          if (!import_fs.default.existsSync(appApiDir)) import_fs.default.mkdirSync(appApiDir, { recursive: true });
...
export async function POST(req: Request) {
  try {
    ...
    const data = await res.json();
    if (!res.ok) {
      return NextResponse.json({ error: data.message || '\xC9chec du paiement' }, { status: res.status });
    }
...
`;
          import_fs.default.writeFileSync(import_path.default.join(appApiDir, "route.ts"), nextRouteContent);
          console.log(source_default.green("\u2713 Next.js App Router Route app/api/ashgate/checkout/route.ts g\xE9n\xE9r\xE9e."));
          const hooksDir = import_path.default.join(projectPath, "hooks");
          if (!import_fs.default.existsSync(hooksDir)) import_fs.default.mkdirSync(hooksDir, { recursive: true });
...
export function useAshgatePayment() {
  ...
  const initCheckout = async (params: {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || '\xC9chec du paiement');
...
`;
          import_fs.default.writeFileSync(import_path.default.join(hooksDir, "useAshgatePayment.ts"), hookContent);
          console.log(source_default.green("\u2713 React Hook hooks/useAshgatePayment.ts g\xE9n\xE9r\xE9."));
          const compDir = import_path.default.join(projectPath, "components");
          if (!import_fs.default.existsSync(compDir)) import_fs.default.mkdirSync(compDir, { recursive: true });
...
export default function AshgateCheckout({ amount = 5000, currency = 'XOF' }) {
  ...
  return (
    <div style={{ maxWidth: '420px', margin: '2rem auto', padding: '1.5rem', background: '#0F172A', color: '#FFF', borderRadius: '12px', fontFamily: 'sans-serif' }}>
      <h2 style={{ fontSize: '1.25rem', fontWeight: 'bold', textAlign: 'center', marginBottom: '1rem' }}>Paiement S\xE9curis\xE9 Ashgate</h2>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        ...
        {provider !== 'stripe' && (
          <div>
            <label style={{ display: 'block', fontSize: '0.875rem', marginBottom: '0.25rem', color: '#94A3B8' }}>T\xE9l\xE9phone Mobile Money</label>
            <input
              type="tel"
              value={phoneNumber}
...
`;
          import_fs.default.writeFileSync(import_path.default.join(compDir, "AshgateCheckout.tsx"), reactNextComponent);
          console.log(source_default.green("\u2713 Composant React components/AshgateCheckout.tsx g\xE9n\xE9r\xE9."));
...
`;
          import_fs.default.appendFileSync(envPath, envVars);
          console.log(source_default.green("\u2713 Fichier .env mis \xE0 jour avec VITE_ASHGATE."));
          const srcDir = import_path.default.join(projectPath, "src");
          const compDir = import_path.default.join(srcDir, "components");
          const hooksDir = import_path.default.join(srcDir, "hooks");
          if (!import_fs.default.existsSync(compDir)) import_fs.default.mkdirSync(compDir, { recursive: true });
          if (!import_fs.default.existsSync(hooksDir)) import_fs.default.mkdirSync(hooksDir, { recursive: true });
...
export function useAshgatePayment() {
  ...
  const initCheckout = async (params: any) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || '\xC9chec du paiement');
...
`;
          import_fs.default.writeFileSync(import_path.default.join(hooksDir, "useAshgatePayment.ts"), reactHookContent);
          console.log(source_default.green("\u2713 React Hook src/hooks/useAshgatePayment.ts g\xE9n\xE9r\xE9."));
        } else {
          console.log(source_default.yellow(`
\u2139 G\xE9n\xE9ration des templates non support\xE9e pour le type : ${detectedType}.`));
        }
        console.log(source_default.bold.green("\n\u{1F389} Projet configur\xE9 avec succ\xE8s !"));
        console.log("Vous pouvez maintenant importer le composant ou helper g\xE9n\xE9r\xE9 pour accepter les paiements.");
      } catch (err) {
        console.error(source_default.red("\n\u2717 Erreur d'\xE9criture de la configuration :"), err.message);
        process.exit(1);
      }
    } finally {
...
async function checkEndpoint(url2, name) {
  const start = Date.now();
  try {
    const response = await axios_default.get(url2, { timeout: 5e3 });
    ...
    if (response.status >= 200 && response.status < 400) {
      ...
      return {
        ...
        title: name,
        status: "warn",
        message: `R\xE9ponse inattendue (HTTP ${response.status})`,
        durationMs
      };
    }
  } catch (err) {
...
function registerDoctorCommands(program3) {
  program3.command("doctor").description("Diagnostiquer le syst\xE8me, la connexion et les services Ashgate Wallet").action(async () => {
    ...
    if (majorNode >= 18) {
      results.push({
        category: "Environnement Syst\xE8me",
    ...
    if (walletConfig.isAuthenticated()) {
      ...
      const hoursLeft = Math.floor(msLeft % 864e5 / 36e5);
      const minutesLeft = Math.floor(msLeft % 36e5 / 6e4);
      const timeStr = daysLeft > 0 ? `${daysLeft}j ${hoursLeft}h` : hoursLeft > 0 ? `${hoursLeft}h ${minutesLeft}min` : `${minutesLeft}min`;
      results.push({
        category: "Authentification Wallet",
        title: "Session Keycloak SSO",
        status: "ok",
        message: `Connect\xE9 (${tokens.email || "Utilisateur"}, valide encore ${timeStr})`
      });
      ...
      results.push({
        ...
        title: "Session Keycloak SSO",
        status: "warn",
        message: "Session expir\xE9e. Reconnexion requise (ashgate auth login)"
      });
    } else {
      results.push({
        category: "Authentification Wallet",
        title: "Session Keycloak SSO",
        status: "warn",
        message: "Non connect\xE9 (ashgate auth login)"
      });
    }
    const rawUrl = walletConfig.get().cloudUrl;
    const cloudUrl = rawUrl && !rawUrl.includes("localhost") && rawUrl !== "https://app.ashgateway.com" ? rawUrl : "https://api.ashgateway.com";
    const cloudServices = [
    ...
    for (const cat of categories) {
      console.log(`
${source_default.bold(cat)} :`);
      const catResults = results.filter((r) => r.category === cat);
      for (const r of catResults) {
        let badge = source_default.green("[OK]  ");
        if (r.status === "warn") {
          badge = source_default.yellow("[WARN]");
          totalWarn++;
        } else if (r.status === "fail") {
          badge = source_default.red("[FAIL]");
          totalFail++;
        } else {
          totalOk++;
        }
        const durationStr = r.durationMs !== void 0 ? source_default.dim(` (${r.durationMs}ms)`) : "";
        console.log(`  ${badge} ${source_default.bold(r.title.padEnd(30))} : ${r.message}${durationStr}`);
      }
    }
    console.log(source_default.dim("\n----------------------------------------------------"));
    if (totalFail === 0 && totalWarn === 0) {
      console.log(source_default.green("Diagnostic termin\xE9 avec succ\xE8s. Tous les services sont op\xE9rationnels.\n"));
    } else if (totalFail === 0) {
      console.log(source_default.yellow(`Diagnostic termin\xE9 avec ${totalWarn} avertissement(s).
`));
    } else {
      console.log(source_default.red(`Diagnostic termin\xE9 avec ${totalFail} erreur(s) et ${totalWarn} avertissement(s).
`));
    }
  });
}
...
// src/commands/pay.ts
var import_qrcode_terminal = __toESM(require_main2());
function registerPayCommands(program3) {
  program3.command("pay").description("Cr\xE9er et tester un paiement directement depuis la ligne de commande").option("-a, --amount <amount>", "Montant du paiement (ex: 5000)", parseFloat).option("-c, --currency <currency>", "Devise (XOF, EUR, USD)", "XOF").option("-p, --provider <provider>", "Fournisseur (fedapay, feexpay, stripe)", "fedapay").option("-e, --email <email>", "Email du client", "client@example.com").option("-f, --firstname <firstname>", "Pr\xE9nom du client", "Client").option("-l, --lastname <lastname>", "Nom du client", "Ashgate").option("--phone <phone>", "Num\xE9ro de t\xE9l\xE9phone (ex: 90000000)", "90000000").option("--operator <operator>", "Op\xE9rateur mobile (mtn, moov, celtiis)", "mtn").option("-d, --description <description>", "Description du paiement", "Paiement via Ashgate CLI").option("--slug <slug>", "Slug du projet Ashgate \xE0 utiliser").option("--open", "Ouvrir automatiquement le lien de paiement dans le navigateur").action(async (options) => {
    requireAuth();
    try {
      const amount = options.amount || 5e3;
      ...
      if (!Array.isArray(projects) || projects.length === 0) {
        console.log(source_default.yellow("\nAucun projet trouv\xE9. Veuillez cr\xE9er un projet avec : ashgate project create"));
        return;
      }
...
Initiation du paiement [Projet: ${selectedProject.name}]...`));
      console.log(`  Fournisseur   : ${source_default.cyan(provider.toUpperCase())}`);
      console.log(`  Montant       : ${source_default.green(amount + " " + currency)}`);
      ...
      console.log(source_default.bold.green("\n[OK] Transaction de paiement initialis\xE9e !"));
      console.log(`  ID Transaction : ${transactionId}`);
      if (paymentUrl) {
        console.log(`  Lien de paiement : ${source_default.underline.blue(paymentUrl)}
`);
        console.log(source_default.bold("QR Code de paiement :"));
        import_qrcode_terminal.default.generate(paymentUrl, { small: true });
        if (options.open) {
          console.log(source_default.dim("\nOuverture dans le navigateur..."));
          await open_default(paymentUrl);
        }
      } else {
        console.log(source_default.yellow("  (Demande de confirmation USSD envoy\xE9e sur le mobile du client)"));
      }
      console.log("");
    } catch (err) {
      console.error(source_default.red("\n[FAIL] \xC9chec du paiement :"), err.response?.data?.message || err.message);
      process.exit(1);
    }
  });
}
...
function registerUninstallCommands(program3) {
  program3.command("uninstall").description("D\xE9sinstaller compl\xE8tement Ashgate CLI et supprimer sa configuration locale").option("-y, --yes", "Confirmer automatiquement sans demander d'interaction").action(async (options) => {
    console.log(source_default.bold("\nD\xE9sinstallation d'Ashgate CLI"));
    console.log(source_default.dim("----------------------------------------------------\n"));
    if (!options.yes) {
      const confirm = await askConfirmation("\xCAtes-vous s\xFBr de vouloir d\xE9sinstaller Ashgate CLI ? (y/N) : ");
      if (!confirm) {
        console.log(source_default.yellow("\nAction annul\xE9e."));
        return;
      }
    ...
    try {
      ...
      if (import_fs2.default.existsSync(configPath)) {
        ...
        console.log(source_default.green("\u2713 Configuration et session locales supprim\xE9es."));
      }
    } catch (err) {
      console.log(source_default.yellow("\u26A0\uFE0F Impossible de supprimer la configuration locale :"), err.message);
    }
    ...
    const possiblePaths = [
      ...
      import_path2.default.join(process.env.HOME || "", ".local", "bin", "ashgate")
    ];
    let removedBinary = false;
    for (const p of possiblePaths) {
      if (p && import_fs2.default.existsSync(p) && !p.includes("node_modules") && !p.endsWith("node")) {
        try {
          import_fs2.default.unlinkSync(p);
          console.log(source_default.green(`\u2713 Executable supprim\xE9 : ${p}`));
          removedBinary = true;
          break;
        } catch (err) {
          console.log(source_default.yellow(`\u26A0\uFE0F Droits insuffisants pour supprimer ${p}. Veuillez ex\xE9cuter : sudo rm -f ${p}`));
        }
      }
    }
    if (!removedBinary) {
      console.log(source_default.dim("Pour finaliser la suppression du binaire ex\xE9cutable, lancez :"));
      console.log(source_default.bold("  sudo rm -f /usr/local/bin/ashgate ~/.local/bin/ashgate\n"));
    }
    console.log(source_default.dim("----------------------------------------------------"));
    console.log(source_default.green("D\xE9sinstallation termin\xE9e avec succ\xE8s.\n"));
  });
}

// src/index.ts
var program2 = new Command();
program2.name("ashgate").description(
  source_default.bold("ashgate-cli") + " \u2014 Interface en ligne de commande pour la plateforme Ashgate\n" + source_default.dim("  Connectez-vous \xE0 votre compte et g\xE9rez votre session.")
).version("3.0.0");
...
registerPayCommands(program2);
registerUninstallCommands(program2);
program2.command("status").description("Afficher l'\xE9tat de la connexion").action(() => {
  const cfg = walletConfig.get();
  const tokens = walletConfig.getTokens();
  console.log(source_default.bold("\nStatut ashgate-cli"));
  console.log(`  Cloud URL     : ${cfg.cloudUrl}`);
  console.log(`  Authentifi\xE9   : ${tokens.accessToken && tokens.expiresAt > Date.now() ? source_default.green("oui") : source_default.red("non")}`);
});
